if has('win32')
    ...
    map <leader><F5> :Dispatch .\ol\bin\Debug\net8.0\ol.exe -S example\main.ol<CR>
    map <leader><F10> :Dispatch .\example\bin\Example.exe<CR>
    map <leader>t<F5> :Dispatch .\ol\bin\Debug\net8.0\ol.exe -S test\main.ol<CR>
    map <leader>t<F10> :Dispatch .\test\bin\tests.exe<CR>
else
    map <leader><F8> :Dispatch dotnet build --no-restore<CR>
